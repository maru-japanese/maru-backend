...
const fill = (id, prompt, answer, choices, context, explanation, speech) => ({ id: "particle-" + id, prompt, answer, choices, context, explanation, speech, instruction: "QUAL PARTÍCULA COMPLETA ESTE MODELO?" });
export const PARTICLE_EXERCISES = [
  fill("topic", "わたし＿学生です。", "は", ["は","を","で","に"], "Eu sou estudante. Apresente o tópico.", "は apresenta aquilo sobre o que se comenta e se pronuncia wa. 学生です informa que a pessoa é estudante.", "わたしは学生です。"),
  fill("water", "水＿飲みます。", "を", ["を","に","の","と"], "Bebo água.", "Água é o que se bebe, o objeto desta ação. を vem depois de 水 e se pronuncia o.", "水を飲みます。"),
  fill("bread", "パン＿食べます。", "を", ["を","が","に","で"], "Como pão. Marque o objeto direto.", "O pão recebe a ação de comer. Nos padrões iniciais, esse objeto é marcado com を.", "パンを食べます。"),
  fill("library", "図書館＿勉強します。", "で", ["で","に","を","の"], "Estudo na biblioteca.", "A biblioteca é onde estudar acontece. Para o local desta ação, usamos で.", "図書館で勉強します。"),
  fill("school", "学校＿行きます。", "に", ["に","で","を","の"], "Vou para a escola. Escolha o destino entre as opções.", "に marca o destino. へ seria outra possibilidade, com foco em direção, mas não está entre as opções desta atividade.", "学校に行きます。"),
  fill("direction", "日本＿行きます。", "へ", ["へ","を","で","の"], "Vou ao Japão. Marque a direção.", "へ indica a direção e se lê e como partícula. Fora desse uso, o kana se lê he.", "日本へ行きます。"),
  fill("book", "わたし＿本です。", "の", ["の","を","で","と"], "É meu livro.", "の liga quem possui à coisa possuída: わたしの本, meu livro.", "わたしの本です。"),
  fill("friend", "友達＿話します。", "と", ["と","を","の","で"], "Converso com um amigo.", "と marca a companhia: com quem você conversa. 友達 significa amigo.", "友達と話します。"),
  fill("also", "わたし＿学生です。", "も", ["も","を","に","で"], "Eu também sou estudante.", "も acrescenta também e substitui は neste padrão. O contexto já mencionou outra pessoa estudante.", "わたしも学生です。"),
  fill("question", "学生です＿。", "か", ["か","を","に","の"], "Você é estudante? Faça uma pergunta educada.", "か no final marca a pergunta neste modelo. Não precisamos inverter as palavras.", "学生ですか。"),
  fill("like", "音楽＿好きです。", "が", ["が","を","で","に"], "Gosto de música. Use o padrão básico de 好き.", "好き é um adjetivo de preferência. O que você gosta é marcado com が neste padrão, diferente do objeto com を em ouvir música.", "音楽が好きです。"),
  fill("train", "電車＿行きます。", "で", ["で","の","を","と"], "Vou de trem.", "Aqui で mostra o meio usado para ir. Não indica o destino, mas o transporte.", "電車で行きます。"),
  fill("time", "三時＿行きます。", "に", ["に","を","と","の"], "Vou às três horas.", "三時 é um horário específico. に marca esse horário no modelo.", "三時に行きます。"),
  fill("cat", "ここに猫＿います。", "が", ["が","を","で","に"], "Há um gato aqui. Apresente o que existe.", "猫 é o sujeito cuja existência se informa. が apresenta essa informação; に já marca o lugar aqui.", "ここに猫がいます。"),
  fill("existence", "図書館＿います。", "に", ["に","で","を","と"], "Estou na biblioteca.", "Com います, に indica onde a pessoa está. Compare com estudar na biblioteca, que usa で.", "図書館にいます。"),
  fill("read", "図書館＿本を読みます。", "で", ["で","に","を","の"], "Leio um livro na biblioteca.", "本を já indica o que é lido. 図書館で acrescenta onde a leitura acontece.", "図書館で本を読みます。"),
  fill("list", "パン＿水を買います。", "と", ["と","の","に","で"], "Compro pão e água.", "と liga os dois substantivos de uma lista: pão e água. を vem depois do conjunto que é comprado.", "パンと水を買います。"),
  fill("teacher", "日本語＿先生です。", "の", ["の","で","を","に"], "É professor(a) de japonês.", "の também especifica o tipo: professor de japonês. Não expressa apenas posse material.", "日本語の先生です。"),
  ...
  fill("where", "駅＿どこですか。", "は", ["は","を","で","に"], "Onde fica a estação? Apresente o tópico.", "A estação é o assunto da pergunta. は apresenta 駅 e どこ pergunta sua localização.", "駅はどこですか。"),
  fill("agreement", "いい天気です＿。", "ね", ["ね","を","に","で"], "O tempo está bom, né? Busque concordância.", "ね convida a compartilhar a percepção. A entonação também ajuda a expressar essa intenção.", "いい天気ですね。"),
  ...
  fill("write", "日本語＿書きます。", "で", ["で","に","と","の"], "Escrevo em japonês.", "で marca o meio ou idioma usado. 日本語で significa em japonês.", "日本語で書きます。"),
  fill("who", "田中さん＿来ます。", "が", ["が","を","で","の"], "Tanaka vem. Responda à pergunta “quem vem?”.", "が identifica quem vem, em resposta ao foco da pergunta. は e が não são intercambiáveis em todo contexto.", "田中さんが来ます。")
];
...
export const LISTENING_EXERCISES = [
  ...
  ...SENTENCES.map(item => ({ id: "listen-sentence-" + item.id, prompt: item.tokens.map(t => t[0]).join("") + "。", speech: item.tokens.map(t => t[0]).join("") + "。", answer: item.prompt.replace(/\s*\([^)]*\)/g,""), listening: true, instruction: "OUÇA E ESCOLHA A FRASE EM PORTUGUÊS", explanation: item.tokens.map(t => t[1]).join(" ") + ". " + item.hint }))
];
